...
/*
This library libsk_b_oled is designed to work without using the hardware i2c support (SDA/SCL pin).
It uses the regular I/O Pin from PORTB to do the i2c communication. Some smaller AVR micro-controller
may not have support for the hardware i2c. This "bit bang" version may not be as reliable as the 
hardware i2c, sometimes we do not have any other choices.
...
PORTB is available for most of the AVR micro-controllers, since atTiny only have PB0 to PB4, 
we will just stick those few Pins.
